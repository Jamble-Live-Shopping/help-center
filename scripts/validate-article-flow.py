...
def _load_xcstrings_snapshot(repo_root: Path) -> dict | None:
    """Load the committed CI xcstrings snapshot at scripts/xcstrings-snapshot.json.

    Format mirrors `_load_xcstrings` output: `{key: {locale: value}}`. Used as
    fallback when JAMBLE_IOS_ROOT is unset (e.g. in CI without the iOS clone).
    Generated by `scripts/snapshot-xcstrings.py` from the iOS source-of-truth.
    Refresh whenever a flow.yml gains a new visible_text source=xcstrings key.
    """
    p = repo_root / "scripts" / "xcstrings-snapshot.json"
    if not p.exists():
        return None
    try:
    ...
    for key, locales in strings.items():
        if not isinstance(locales, dict):
            continue
        bucket: dict[str, str] = {}
        for loc_key, loc_val in locales.items():
            if isinstance(loc_val, str):
                bucket[loc_key] = loc_val
        if bucket:
            out[key] = bucket
    return out


def _resolve_xcstrings_data(
    ios_root: Path | None, repo_root: Path
) -> tuple[dict | None, str]:
    """Resolve xcstrings data with priority order.

    1. JAMBLE_IOS_ROOT Localizable.xcstrings (full source-of-truth)
    2. Committed CI snapshot at scripts/xcstrings-snapshot.json (CI fallback)
    3. None (validator emits one soft warn per article ; xcstrings_locale_drift
       NOT enforced for source=xcstrings entries on that article)

    Returns (xc_data, source) where source ∈ {"ios_root", "snapshot", ""}.
    """
    if ios_root is not None:
        data = _load_xcstrings(ios_root)
        if data is not None:
            return data, "ios_root"
    snap = _load_xcstrings_snapshot(repo_root)
    if snap is not None:
...
def _validate_visible_text(
    ...
    Rules emitted:
      ...
      visible_text_resolve_error    — entry can't be resolved (HARD), excluding
                                  the "no xcstrings source" case which becomes
                                  a SOFT warn (xcstrings_resolution_skipped).
      xcstrings_resolution_skipped  — ONE soft warn per article when xc_data is
                                  None: selectors still mark elements as covered
                                  (no cascade into visible_text_uncovered), but
                                  xcstrings_locale_drift is NOT enforced for
                                  those entries. Sourced fallback order:
                                  JAMBLE_IOS_ROOT > scripts/xcstrings-snapshot.json.
    ...
    # Track xcstrings entries that were skipped because no source resolved.
    # We emit ONE summary soft warn per article (not per entry, not per screen)
    # so the reviewer sees the gap exactly once.
    xc_skipped_total = 0
        ...
        for locale in ("en", "pt-br"):
            ...
            for entry in visible_text:
                ...
                if not matches:
                    ...
                    continue
                # Soft-degrade path: xcstrings source unavailable. Mark elements
                # as covered (no visible_text_uncovered cascade), bump skip
                # counter for the summary warn, and proceed without drift check.
                if entry.get("source") == "xcstrings" and xc_data is None:
                    for el in matches:
                        covered.add(id(el))
                    xc_skipped_total += 1
                    continue
    ...
    # ONE summary soft warn per article when xcstrings entries were skipped
    # because no source resolved. Selectors were still marked as covered, but
    # xcstrings_locale_drift was NOT enforced for those entries. Reviewer
    # sees the gap exactly once with the resolution-source breadcrumb.
    if xc_skipped_total > 0:
        rep.warn(
            "xcstrings_resolution_skipped",
            f"{slug}: {xc_skipped_total} visible_text source=xcstrings entry/entries "
            f"could NOT be resolved (xc_source='{xc_source or 'none'}'). "
            f"xcstrings_locale_drift NOT enforced for those entries. "
            f"Set JAMBLE_IOS_ROOT (full source) OR ensure "
            f"scripts/xcstrings-snapshot.json is committed and contains the "
            f"declared keys (regen via `scripts/snapshot-xcstrings.py`).",
        )
...
def validate_article(article_dir: Path) -> Report:
    ...
    # Resolve xcstrings via priority: JAMBLE_IOS_ROOT > committed CI snapshot > none.
    xc_data, xc_source = _resolve_xcstrings_data(ios_root_path, REPO_ROOT)
    _validate_visible_text(rep, article_dir, flow, mockup_dir, xc_data, xc_source)
